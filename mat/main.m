...
sgram_span_count = size(sgram_span,1);  %单位次  总共采集多少个波段 
acq_time = 2; %单位s    sgram模式下单次测量时间

spctrum_acq_period = 60;   %单位秒      %count 计数50次大约20s   
spectrum_Execute_time = 1;  %单位次    spectrum执行多少次
sgram_spec_delt_time = 5;   %sgram测量完成后 等待多少s执行开始spec测量任务
...
elseif strcmp(mode,'sgram')
    ...
    fprintf(myRSA,'SENSE:SGRAM:BANDWIDTH:RESOLUTION:AUTO OFF'); %手动设置rbw
    fprintf(myRSA,['SENSE:SGRAM:BANDWIDTH:RESOLUTION ',num2str(sgram_rbw),'HZ']);
    fprintf(myRSA,'SENSE:SGRAM:WATERFALL:ENABLE ON'); %打开3d显示模式
...
global sgram_frame_id;
global frame_id;
disp(['%%%%%%%%%%%%%%%%%%%%%%%%%开始第',num2str(sgram_frame_id),'次sgram测量%%%%%%%%%%%%%%%%%%%%%%%%%%%%              task_count:第',num2str(frame_id),'次']);
...
acqtimer = timer('Name','acq_timer','TimerFcn',@acq_TimerFcn,'StartDelay',acq_time,'ExecutionMode','singleShot');  %启动定时器
start(acqtimer);
wait(acqtimer);
end
...
fprintf(myRSA,'DISPLAY:GENERAL:MEASVIEW:SELECT SGRam');  %选择sgram窗口
time = strrep(datestr(now),':','_');   %csv文件名不能有：  选择替换为_
...
t2=clock;
disp(['本次SGRAM保存时间',num2str(etime(t2,t1)),'S']);
fprintf(myRSA,'DISPlay:GENeral:MEASview:delete SGRam');
