...
for i=2:(length(l)-1)
    ...
    D = [D  waverec(c_new, l, ['db', num2str(y_max)])];
...
f = Fs/n*(0:n/5);
figure('color','w');
plot(f,Pyy(1:n/5+1) , 'color', 'black', 'LineWidth', 1.5)
title('近似信号a 频谱强度','FontSize',18)
...
for i =1 :size(D,2)
    ...
    title(['细节信号D' num2str(size(D,2) - i +1) '频谱强度'],'FontSize',18)
    hold on;
end
xlabel('频率 [1/24h]','FontSize',18);