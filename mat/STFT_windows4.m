...
%% 显示[上下午]标签文字
yLabels = {'0-6', '6-12', '12-18', '18-24'};  % 待添加的标签
...
for i = 0 : length(yLabels)*7-1
    ...
    text(1/lenlab * i+0.06+offset, 1-0.05, yLabels(mod(i,4)+1));   % 用文本的方式添加，位置可以自定义
...
size(s, 2)
for i=1:size(s, 2)
    MS = [MS mean( data_yuan((i-1)*fs/4+1:i*fs/4) )];
end
...
reshape_mat = reshape(FC, 4, 7);  %方便求每天FC最大值
[max1, loc_max] = max(reshape_mat,[],1);   %max  最大值  loc最大值位置
[min1, loc_min] = min(reshape_mat,[],1);   %min  最小值  loc最小值位置
for i = 1 : size(FC, 2)
    if mod(i-1, 4)+1 == loc_max(floor((i-1)/4)+1)
        % 每天的最大值时间段高亮显示
        text(1/lenlab * (i-1)+0.045, 1+0.05, num2str(FC(1, i),'%.3f'),'backgroundcolor', [255,64,0]./255 , 'edgecolor', 'white','FontSize',10); 
    elseif mod(i-1, 4)+1 == loc_min(floor((i-1)/4)+1)
        text(1/lenlab * (i-1)+0.045, 1+0.05, num2str(FC(1, i),'%.3f'),'backgroundcolor', [0,191,255]./255, 'edgecolor', 'white','FontSize',10);     
    else
        text(1/lenlab * (i-1)+0.045, 1+0.05, num2str(FC(1, i),'%.3f'),'backgroundcolor', [154,254,46]./255, 'edgecolor', 'white','FontSize',10);   % 用文本的方式添加
    end
end
...
for i = 1 : size(s, 2)
    ...
    text(1/lenlab * (i-1)+0.045, 1+0.05*2+0.01, context,'edgecolor', 'white','FontSize',8);
end
